# Taiwan Election Interactive Map (台灣選舉地圖)

## Overview

A Taiwan election interactive map with historical year switching (2024/2020/2016), a PVI (黨派投票指數 / composite partisan-voting-index) mode, scenario simulation (動態模擬), and a party-list-only county/township/village map with its own "推演" simulated year (§13 below). It is a ~13MB, ~362,000-line HTML file (`test.html`) with inline CSS and JavaScript, rendered with D3.js v7 for the map/SVG layer. There is no build system, no package manager, and no server requirement.

**Data layer (as of 2026-08-31 slimming pass):** geographic/election data lives entirely in external JS modules loaded via `<script src>` — `data_map_v64.js` (county/village/township/district boundary topology + name lookups: `CD`, `VD`, `NM`, `ED_GJ`, `TC_GJ`, `ED_GJ_2016`, `TC_GJ_2016`; ~55MB), `data_vd_v64.js` (village-level simulation/2016 data: `VC_SIM`, `VC_2016`; ~7MB), `data_region_v64.js` (party-list vote history across 5 cycles: `PL08_VC`/`PL12_VC`/`PL16_VC`/`PL_VC_2020`/`PL_VC_2024` + turnout tables; ~6.5MB), and `data_vc_ed_legis.js` (`VC_ED`, this lineage's own 2024 village-level legislative results — **not** the same values as `council.html`/`live2026.html`'s `data_vc_ed_council.js`, see the file's own header comment; a real data discrepancy between the two was found and deliberately left unmerged, not yet root-caused). All 3 shared files are byte-identical to `council.html`/`party_list.html`/`live2026.html`'s own copies of the same modules — loaded once, cached by the browser across all 4 pages, which is what makes cross-page navigation fast after the first visit. Nothing election-data-sized is embedded inline in the HTML anymore; see `CHANGELOG.md` v2.98 for how this was verified before shipping.
